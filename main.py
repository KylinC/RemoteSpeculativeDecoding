...
def parse_arguments():
    ...
    parser.add_argument('--tokenizer_name', type=str, default=MODELZOO["llama2-70b"])
...
def generate(input_text, approx_model_name, target_model_name, tokenizer_name, num_tokens=20, gamma = 4,
    ...
    output = large_model.generate(input_ids, max_length=200,do_sample=True,temperature=1,top_k=top_k,top_p=top_p) 
    generated_text = tokenizer.decode(output[0], skip_special_tokens=True)
    color_print(f"large (target) model autoregressive_sampling (standard TF): {generated_text}")
...
if __name__ == "__main__":
    ...
    generate(args.input, args.approx_model_name, args.target_model_name, args.tokenizer_name , num_tokens=args.max_tokens, gamma=args.gamma,
